...
const getDefaultCourse = (slug: string) => ({
  ...
  description: `Master ${slug.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")} with 100% practical training at WebiGeeks. Build real-world projects and get placement assistance.`,
  technologies: [],
  curriculum: [],
  projects: [],
  careers: [],
...
export default function CourseDetailContent({ slug }: { slug: string }) {
  const course = courseData[slug] || getDefaultCourse(slug);
            ...
            <div className="lg:col-span-2">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex flex-wrap gap-2 mb-4">
                  <span className={`px-3 py-1 rounded-full bg-gradient-to-r ${course.color} text-xs font-bold`}>
                    {course.level}
                  </span>
                ...
                {/* Stats bar for detailed courses */}
                {course.stats && course.stats.length > 0 && (
                  <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {course.stats.map((s: { label: string; value: string }) => (
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-xl border border-border overflow-hidden">
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-text-primary">{mod.module}</span>
                    </div>
        ...
        <section className="section-padding bg-white">
          <div className="container-custom">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-4xl mx-auto">
              {course.projects.map((p: any, i: number) => (
