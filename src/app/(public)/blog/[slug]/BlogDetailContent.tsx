      ...
      <section className="bg-ink">
        <div className="container-custom pt-16 pb-12 md:pt-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            />
            <Link href="/blog" className="inline-flex items-center gap-1 text-white/50 hover:text-white text-sm mb-6 transition-colors">
              <ArrowLeft className="w-4 h-4" /> Back to Blog
            </Link>
            <div className="flex items-center gap-4 font-mono-agency text-xs text-white/50 mb-6 tracking-wide">
            ...
            <h1 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white leading-[1.1]">
              {blog.title}
            </h1>
          </motion.div>
        </div>
        ...
        <div className="container-custom pb-24 md:pb-32">
          <div className="max-w-3xl mx-auto">
            <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="bg-ink-soft rounded-[32px] overflow-hidden">
                ...
                <div className="p-8 md:p-10">
                  <div
                    ...
                    className="prose prose-invert prose-lg max-w-none mb-6 text-white/80 leading-relaxed prose-headings:text-white prose-headings:font-display prose-a:text-agency-accent prose-strong:text-white prose-blockquote:border-agency-accent"
                  />
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-white/10">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-ink font-bold">W</div>
                      <div>
                        <p className="font-semibold text-white text-sm">WebiGeeks Digital</p>
                        <p className="text-xs text-white/50">Published {formatDate(blog.publishedAt || blog.createdAt)}</p>
                      </div>
                    </div>
                    <button className="flex items-center gap-1.5 text-white/50 hover:text-agency-accent text-sm transition-colors">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {relatedBlogs.map((relatedBlog) => (
                    <Link
                      ...
                      className="group p-4 rounded-2xl border border-white/10 bg-ink-soft hover:border-agency-accent transition-colors"
                    >
                      <h3 className="font-semibold text-white group-hover:text-agency-accent transition-colors line-clamp-2">
                        {relatedBlog.title}
                      </h3>
                      <p className="font-mono-agency text-xs text-white/50 mt-2">{relatedBlog.category}</p>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            {/* CTA */}
            <div className="mt-10 p-10 rounded-[32px] bg-ink-soft text-center">
