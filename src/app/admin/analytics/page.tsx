...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const [overviewRes, admissionsRes, revenueRes, leadsRes, coursesRes, studentsRes] = await Promise.all([
          fetch(`${API_URL}/analytics/overview`, { headers }),
          fetch(`${API_URL}/analytics/admissions?year=${year}`, { headers }),
          fetch(`${API_URL}/analytics/revenue?year=${year}`, { headers }),
          fetch(`${API_URL}/analytics/leads?year=${year}`, { headers }),
          fetch(`${API_URL}/analytics/courses`, { headers }),
          fetch(`${API_URL}/analytics/students?year=${year}`, { headers }),
        ]);

        const [overviewJson, admissionsJson, revenueJson, leadsJson, coursesJson, studentsJson] = await Promise.all([
          overviewRes.json(), admissionsRes.json(), revenueRes.json(), leadsRes.json(), coursesRes.json(), studentsRes.json(),
        ]);

        if (!overviewRes.ok) throw new Error(overviewJson.message || "Could not load analytics");

        setOverview(overviewJson.data);
        setAdmissions(admissionsJson.data || []);
  ...
  const revenueChartData = (revenue?.byMonth || []).map((d) => ({ month: MONTH_LABELS[d.month - 1], total: d.total }));
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <ChartCard title="Student Status" subtitle="Pending / approved / rejected">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={students?.statusBreakdown} dataKey="count" nameKey="status" cx="50%" cy="50%" outerRadius={75} label={(entry: any) => entry.status}>
        ...
        <ChartCard title="Payment Mode" subtitle="Full payment vs. EMI">
          {(students?.paymentModeBreakdown?.length || 0) === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={students?.paymentModeBreakdown} dataKey="count" nameKey="mode" cx="50%" cy="50%" outerRadius={75} label={(entry: any) => entry.mode}>
